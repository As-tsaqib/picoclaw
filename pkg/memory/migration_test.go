...
func TestMigrateFromJSONPreservesNamedSessionMetadataAndActiveMapping(t *testing.T) {
	...
	scope := json.RawMessage(
		`{"version":1,"agent_id":"main","channel":"telegram","account":"bot-a",` +
			`"dimensions":["chat"],"values":{"chat":"direct:42"}}`,
	)
	...
	if writeErr := os.WriteFile(
		filepath.Join(sessionsDir, activeSessionsFilename), activeData, 0o600,
	); writeErr != nil {
