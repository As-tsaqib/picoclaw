...
func TestMemoryEvolutionManagementAPIRejectsArbitrarySelectorsAndMalformedInput(t *testing.T) {
	...
	tests := []struct {
		...
		{http.MethodPost, "/api/memory/workspace", `{"action":"add","content":"safe","type":"project_fact","user_id":"other"}`},
		{http.MethodPost, "/api/memory/workspace", `{"action":"remove","id":"../../private"}`},
		{http.MethodPost, "/api/memory/pending/not-a-pending-id/approve", "{}"},
		{http.MethodPost, "/api/memory/pending/pm_0011223344556677/approve?user_id=other", "{}"},
...
func TestEvolutionManagementAPIPreviewApproveApplyVersionsRejectAndRollback(t *testing.T) {
	...
	drafts := []evolution.SkillDraft{
		{
			ID: "draft-api-apply", WorkspaceID: harness.workspace, CreatedAt: now,
			SourceRecordID: pattern.ID, TargetSkillName: "remote-validation",
		...
		{
			ID: "draft-api-reject", WorkspaceID: harness.workspace, CreatedAt: now.Add(time.Second),
			SourceRecordID: pattern.ID, TargetSkillName: "remote-validation-rejected",
			DraftType: evolution.DraftTypeWorkflow, ChangeKind: evolution.ChangeKindCreate,
